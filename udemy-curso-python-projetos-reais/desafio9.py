...
if __name__ == '__main__':
    # Impressão da lista de lista de números inteiros
    # print(lista_de_listas_de_inteiros)

    # Contador de listas
    contador = 1

    # Itera a lista de lista de números inteiros
    for nlista in lista_de_listas_de_inteiros:

        print(f'*** Lista {contador} ***')

        numeros_duplicados = set()
        # Itera os números da lista para poder identificar os duplicados
        for i, n in enumerate(nlista):
            temp = nlista[i + 1:]
            # print(temp)
            if n in temp:
                numeros_duplicados.add(n)

        # Retorna -1 se o set estiver vazio, caso contrário retorna os elementos duplicados
        if len(numeros_duplicados) == 0:
            print('-1\nNão tem duplicação')
        else:
            print(*numeros_duplicados, sep=', ')

        # Incrementa o contador
        contador += 1
        print('')  # Separador de linha, para melhorar a visualização do resultado
